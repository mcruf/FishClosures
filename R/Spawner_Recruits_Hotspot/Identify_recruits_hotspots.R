...
# Neuenfeldt & Köster (Trophodynamic control on recruitment success in Baltic cod: the influence of cannibalism)
# define recuits as follows:

  #Recruitment refers to the beginning of the 3rd quarter (0-group) or to the 1st of January (1- and 2-group). 
 


# To identify recruits hotpspot, I will have to take all the months into consideration, in opposit to
# the Spawner hotspot, where I knew the monhts corresponding to spawning.
# To do so, I have to do the same 3 streps as in for the spawners:
...
#write.csv(treshold2,"Recruits_Abundance_treshold.csv")

#Load saved object from the tresholds
treshold2 <- read.csv("Recruits_Abundance_treshold.csv")
treshold2[,1]<-NULL
...
persistency2$Index <- persistency2$sum/180 #180=number of time-steps (columns)
...
# Draw polygons of the hotspot areas
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
dens <- density(Xaxis[,178]) #Select an arbitray example
trs <- treshold2[178,] #Select number according to the above one
...
ggsave("C:/Users/mruf/OneDrive - Danmarks Tekniske Universitet/PhD/Manuscript_03/Figures/Recruits_CRDFs.png",
